program sh_expand
  ...
  case('int')
    ...
    allocate(grid(n * n * 2), coeff((lmax_calc + 1) * (lmax_calc + 2) - lmax_calc - 1))
    ...
    do i = 0, lmax_calc
      do j = 0, i
        cilm(1, i + 1, j + 1) = coeff(indx_2(i + 1, j + 1))
        cilm(2, i + 1, j + 1) = coeff(indx_2(i + 1, j + 1) + 1)
      end do
    end do
    write(stdout, '(1x, a)') ' done!'
  end select
...
n_lat = int(180._8 * 0.5_8, kind = 4)
n_lon = n_lat * 2
allocate( PB(n_leg), Pnm(n_coeff) )
! data display
!status_i = 100.0 / n_lat
! progress bar
!progress_bar_string = '   Completed %: '
...
do i = 1, n_lat
  ...
  Bi = Bi - lat_step
! progress bar
  !do ii = 1, len(progress_bar_string) + 5
    !write(6, '(a)', advance = 'no') '\b'
  !end do
  !write(6, '(a)', advance = 'no') progress_bar_string
  !write(6, '(f5.1)', advance = 'no') real(i, 4) * status_i
end do
!write(6,'(a)')
call cpu_time(t_end)
